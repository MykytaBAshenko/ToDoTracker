...
function NewToCalendar(props) {
    ...
    const [list_of_users_for_meeting, setlist_of_users_for_meeting] = useState([auth?.user?.email])
    ...
    const addUserIfExist = () => {
        axios.post('/api/calendar/checkemail', {
            ...
            if(d.data.success) {
                setlist_of_users_for_meeting([...list_of_users_for_meeting, d.data.user.email])
    ...
    useEffect(()=> {
        setlist_of_users_for_meeting([auth?.user?.email])
